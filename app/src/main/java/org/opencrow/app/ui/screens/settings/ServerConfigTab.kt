...
fun ServerConfigTab(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(spacing.md)) {
        when (tab) {
            ...
            "email" -> EmailConfigPanel(cfg, onConfigChange)
            "channels" -> ChannelsConfigPanel(cfg, onConfigChange)
            "devices" -> DevicesConfigPanel(cfg, onConfigChange)
            "tools" -> ToolsConfigPanel(cfg, onConfigChange)
            "skills" -> SkillsConfigPanel(cfg, onConfigChange)
            "mcp" -> McpConfigPanel(cfg, onConfigChange)
            "servers" -> ServersConfigPanel(cfg, onConfigChange)
            "schedules" -> SchedulesConfigPanel(cfg, onConfigChange)
        }
